...
// Logika filter gabungan: status stok + kata kunci pencarian
function getFilteredProducts(state) {
  return state.products.filter((p) => {
    // 1. Filter Stok
    const matchesFilter =
      state.filter === 'all' ||
      (state.filter === 'available' && p.stock > 0) ||
...
// Event delegation tombol "Tambah ke Keranjang"
